"""SDXL + ControlNet backend — rotoscoping and style transfer."""
from __future__ import annotations

import gc
import logging
import time
from pathlib import Path
from typing import List, Optional, Tuple

from backend.services.prompt.types import ComposedPrompt
from backend.services.video.backends.base import VideoBackend
from backend.services.video.types import VideoClip

logger = logging.getLogger("beat_studio.video.sdxl_controlnet")

_SUPPORTED_STYLES = {
    "rotoscope",        # Primary use case (requires source footage)
    "comic_book",
    "pop_art",
    "ukiyo_e",
    "synthwave",
}
_DEFAULT_MODEL_PATH = "backend/models/sdxl/sd_xl_base_1.0.safetensors"
_VRAM_GB = 8.0


class SDXLControlNetBackend(VideoBackend):
    """SDXL + ControlNet local backend for rotoscoping and style transfer.

    Best for: Applying artistic style to existing video/images (rotoscope).
    Requires source footage as ControlNet conditioning input.
    VRAM: ~8GB
    Cost: Free (local GPU)
    """

    def __init__(self, model_path: Optional[str] = None):
        self._model_path = Path(model_path or _DEFAULT_MODEL_PATH)
        self._pipeline = None

    def name(self) -> str:
        return "sdxl_controlnet"

    def vram_required_gb(self) -> float:
        return _VRAM_GB

    def supports_style(self, style: str) -> bool:
        return style in _SUPPORTED_STYLES

    def is_available(self) -> bool:
        return self._model_path.exists()

    def estimated_time_per_scene(self, resolution: Tuple[int, int] = (1024, 1024)) -> float:
        return 60.0  # ~1 min per scene with 30-step SDXL

    def estimated_cost_per_scene(self) -> float:
        return 0.0

    def generate_clip(
        self, prompt: ComposedPrompt, duration_sec: float,
        resolution: Tuple[int, int], fps: int = 24, seed: int = -1,
    ) -> VideoClip:
        t0 = time.time()
        out_path = self._run_pipeline(prompt, duration_sec, resolution, fps, seed)
        return VideoClip(
            file_path=out_path, duration_sec=duration_sec,
            width=resolution[0], height=resolution[1], fps=fps,
            scene_index=-1, backend_used=self.name(),
            generation_time_sec=time.time() - t0, cost_usd=0.0,
        )

    def generate_batch(
        self, prompts: List[ComposedPrompt], durations: List[float],
        resolution: Tuple[int, int], fps: int = 24,
    ) -> List[VideoClip]:
        return [self.generate_clip(p, d, resolution, fps) for p, d in zip(prompts, durations)]

    def kill(self) -> None:
        del self._pipeline
        self._pipeline = None
        gc.collect()
        try:
            import torch
            torch.cuda.empty_cache()
        except Exception:
            pass

    def _run_pipeline(
        self, prompt: ComposedPrompt, duration_sec: float,
        resolution: Tuple[int, int], fps: int, seed: int,
    ) -> str:
        raise NotImplementedError("SDXL+ControlNet pipeline not loaded.")
